...
class IAPHelper : NSObject {
    ...
    enum IAPSubscriptionTypes: String{
        case subsciptionMonthly = "soundtrack.sub.monthly"
        case subsciptionYearly = "soundtrack.sub.yearly"
    }
    
    public static var shared : IAPHelper? = IAPHelper()
    private let sharedSecret = "babf70edc79e4520827d08b44b698f0d"
    ...
    func restorePurchase() {
        ...
        SwiftyStoreKit.restorePurchases(atomically: true) { [self] results in
            if (results.restoreFailedPurchases.count>0) {
                ...
                delegate?.errorAlert("Something went wrong")
            }
